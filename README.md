...
| **API** | Регистрация/логин JWT, вопросы, ответы, лента, сравнение, лайки, личный чат (`/conversations`), групповой чат (`/group-rooms`); **`GET /users/{id}`** — поле **`conversation_id`** при взаимном матче; **непрочитанные:** **`GET /conversations/unread-count`**, **`POST /conversations/{id}/read`**, **`POST /group-rooms/{id}/read`** (таблицы read state, Alembic **`0017`**); **Threads-like** (`thread_posts`): `/timeline`, `/posts/{id}`, `/posts/{id}/replies`, `/posts`, `/posts/{id}/reply`, `/posts/{id}/can-reply`, лайки/репост/цитата; **темы** `/timeline?topic=...` + `/axes`; **`GET /users/{id}/threads`**, **аватары** (`POST/DELETE /me/avatar`, `GET /users/{id}/avatar`); **блок/жалоба** (`POST/DELETE /users/{id}/block`, `POST /users/{id}/report`), `PATCH /me/profile` |
| **UI** | Тёмная тема, **`mm-*`**, **Geist**, **`BottomNav`** на ленте, профиле (`/summary`), чужом профиле (`/users/[id]`), чатах (`/chat`, `/group-chat`, список диалогов); **выход** из аккаунта на `/summary`; **бейдж непрочитанных** на иконке «Чаты»; онбординг, тест, summary, **`/terms`**, **`/privacy`**, **Threads-like** (`/timeline`, `/posts/[id]`, `/threads/[id]`, composer); профили с «как думает», активностью; при матче — **«Чат»** вместо лайка; **`avatarPublicSrc`** в `lib/api.ts` для отображения аватаров через прокси `/api` |
