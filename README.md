...
| Слой | Сделано |
|------|---------|
| **Данные** | Пользователи, вопросы (**два пакета:** `pack=onboarding` — первый блок, `pack=onboarding_plus` — добровольный второй блок из 10 вопросов после завершения первого), оси (`question_axes`), связи вопрос–ось, ответы, лайки; **личные** чаты (матч → `conversations`/`messages`); **групповые** комнаты (`group_rooms`, участники, сообщения, жалобы) |
...
| **API** | Регистрация/логин JWT, **`GET /questions?pack=…`** (разрешены только `onboarding` и `onboarding_plus`), ответы, лента, сравнение, лайки, личный чат (`/conversations`), групповой чат (`/group-rooms`); **`GET /me/summary`** — в т.ч. **`onboarding_plus_total`** / **`onboarding_plus_answered`** для второго блока; **`GET /users/{id}`** — поле **`conversation_id`** при взаимном матче; **непрочитанные:** **`GET /conversations/unread-count`**, **`POST /conversations/{id}/read`**, **`POST /group-rooms/{id}/read`** (таблицы read state, Alembic **`0017`**); **Threads-like** (`thread_posts`): `/timeline`, `/posts/{id}`, `/posts/{id}/replies`, `/posts`, `/posts/{id}/reply`, `/posts/{id}/can-reply`, лайки/репост/цитата; **темы** `/timeline?topic=...` + `/axes`; **`GET /users/{id}/threads`**, **аватары** (`POST/DELETE /me/avatar`, `GET /users/{id}/avatar`); **блок/жалоба** (`POST/DELETE /users/{id}/block`, `POST /users/{id}/report`), `PATCH /me/profile` |
| **UI** | Тёмная тема, **`mm-*`**, **Geist**, **`BottomNav`** на ленте, профиле (`/summary`), чужом профиле (`/users/[id]`), чатах (`/chat`, `/group-chat`, список диалогов); **выход** из аккаунта на `/summary`; **бейдж непрочитанных** на иконке «Чаты»; онбординг, **`/test`** (первый блок) и **`/test?pack=onboarding_plus`** (второй блок, только после `test_completed`), summary с блоком **«Профиль»** и кнопкой **«Продолжить опрос»**, **`/terms`**, **`/privacy`**, **Threads-like** (`/timeline`, `/posts/[id]`, `/threads/[id]`, composer); профили с «как думает», активностью; при матче — **«Чат»** вместо лайка; **`avatarPublicSrc`** в `lib/api.ts` для отображения аватаров через прокси `/api` |
| **Демо** | `python seed.py` — оси, первый блок вопросов и пак **`onboarding_plus`** (идемпотентно); `python scripts/seed_fixture_users.py` — симуляция (**100** пользователей по умолчанию, **случайные** ответы; пароль у всех один — `ChaosDemo2026!`, см. константу в скрипте; `--count`, `--domain`) |
...
- **Второй блок вопросов (`onboarding_plus`):** данные в `backend/app/seed_data/onboarding_plus.py`; при каждом запуске **`python seed.py`** синхронизируется и первый пак, и плюс-пак (даже если онбординг уже в БД). После прохождения первых 10 вопросов пользователь может пройти ещё 10 с экрана **`/summary`** (блок «Профиль») или по ссылке **`/test?pack=onboarding_plus`**. Шаг **`onboarding_step`** меняется только после первого блока; ответы второго блока дополняют осевые скоры. На Render после деплоя с новым кодом достаточно того же `seed.py` в стартовой команде (как в `render.yaml`).
- **Нижняя навигация:** компонент **`BottomNav`** подключён также на страницах **чужого профиля** (`/users/[id]`), **своего профиля** (`/summary`), **личного** (`/chat/[id]`) и **группового** (`/group-chat/[id]`) чата (раньше пропадала на этих маршрутах).
- **Профиль / аккаунт:** на **`/summary`** добавлен **выход** (очистка JWT и переход на `/login`). В ответе **`GET /users/{id}`** при взаимном матче приходит **`conversation_id`** — на карточке пользователя показывается **«Чат»** вместо повторного «Интересно (лайк)».
- **Аватары в UI:** в **`frontend/lib/api.ts`** добавлена **`avatarPublicSrc()`** — относительные URL вида `/users/…/avatar` превращаются в **`/api/…`**, чтобы запрос шёл через прокси Next к бэкенду (иначе браузер бьётся в origin фронта и картинка не открывается). Используется в ленте людей и профилях.
- **Непрочитанные сообщения:** таблицы **`conversation_read_states`** и **`group_room_read_states`** (миграция Alembic **`0017`**; на SQLite при старте API при необходимости создаются из **`ensure_sqlite_chat_read_state_tables`** в `database.py`). Эндпоинты: **`GET /conversations/unread-count`** (сумма по личкам и группам), пометка прочитанного — **`POST /conversations/{id}/read?last_message_id=…`**, **`POST /group-rooms/{id}/read?last_message_id=…`**. В **`frontend/components/BottomNav.tsx`** — опрос счётчика и **красный бейдж** на вкладке «Чаты». Список **`GET /conversations`** дополняется полем **`unread_count`** по каждой строке (личка/группа) — на **`/conversations`** красный бейдж у конкретного диалога.
- **Логика «прочитано» и уведомления:** на **`/chat/[id]`** и **`/group-chat/[id]`** сообщения считаются прочитанными **только если** список прокручен **почти к низу** (порог ~96px); при опросе новых сообщений **автопрокрутка вниз** — только если пользователь уже был у низа; после **своей** отправки — как «внизу». Во **вкладке в фоне** (`document.hidden`) при входящем от других — короткий **звук** (Web Audio) и **браузерное уведомление**, если пользователь выдал разрешение (кнопка **«Уведомления»** в шапке чата). Общие хелперы: **`frontend/lib/chatClient.ts`**.
...
| `frontend/lib/api.ts` | `apiUrl`, JWT, **`avatarPublicSrc`** (аватары через `/api/…`), **повторы GET** при 502/503/504 и сетевых ошибках; `apiJsonWithEtag` для поллинга |
| `frontend/lib/chatClient.ts` | Проверка «у низа» скролла, звук/уведомление о новом сообщении во вкладке в фоне |
...
5. **После MVP:** OAuth, PWA/push, ветвления вопросов и дополнительные паки по сценариям (лайк → N вопросов и т.д.).
