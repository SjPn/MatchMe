...
- **Редизайн UI:** переход на **светлую** палитру и **синий** акцент вместо тёмной темы и зелёного (`emerald`). Стили: `frontend/app/globals.css`, тени в `tailwind.config.ts`. Для ассистентов ИИ — краткий контекст репозитория: **`AGENTS.md`** (читать при новой сессии).
- **Уведомления о сообщениях:** двухтоновый звук в **`lib/chatClient.ts`**, системные **Notification** (без `silent`, чтобы ОС могла проигрывать звук на телефоне/десктопе при разрешении), опциональная вибрация на Android; **`GlobalChatAlerts`** в корневом `layout` опрашивает **`GET /conversations/unread-count`** на экранах вне чата, чтобы при росте непрочитанных во **вкладке в фоне** тоже сработало оповещение. На открытом чате логика остаётся в **`useChatPolling`**.
- **Чат:** область сообщений с **`flex-1 min-h-0`**, чтобы скролл был у контейнера и при открытии диалога показывались **последние** сообщения (иначе flex растягивал блок на всю высоту ленты).
...
- **Логика «прочитано» и уведомления:** на **`/chat/[id]`** и **`/group-chat/[id]`** сообщения считаются прочитанными **только если** список прокручен **почти к низу** (порог ~96px); при опросе новых сообщений **автопрокрутка вниз** — только если пользователь уже был у низа; после **своей** отправки — как «внизу». Звук уведомления также при **прокрутке вверх** в том же чате (новое сообщение не на экране). Во **вкладке в фоне** — звук + **Notification** (см. **`GlobalChatAlerts`** и **`chatClient.ts`**). Кнопка **«Уведомления»** в шапке чата запрашивает разрешение браузера.

### Документация в репозитории

| Файл | Назначение |
|------|------------|
| **`AGENTS.md`** | Контекст проекта для ИИ/разработчиков: стек, UI, важные пути, что уже сделано. |
| **`README.md`** | Этот файл: запуск, деплой, состояние MVP, производительность. |
| **`CONCEPT.md`** | Продуктовая концепция и позиционирование. |
| **`PRODUCT_VISION.md`** | Видение продукта (нейтрально по стеку). |
| **`MVP_FLOW.md`** | Экраны MVP и логика флоу. |
| **`TODO.md`** | Чеклист следующих шагов. |
| **`frontend/README.md`** | Кратко про фронтенд Next.js в этом монорепо. |
...
**Дизайн и вёрстка:** **светлая тема**, акцентные цвета **`sky-*`** и CSS‑переменные **`--mm-accent`** в `globals.css`; переиспользуемые классы **`mm-page`**, **`mm-input`**, **`mm-btn-primary`** и др.; ширина колонки **`max-w-shell`** / **`lg:max-w-shell-wide`** (`tailwind.config.ts`). Корневой `layout` подключает **Geist** (`app/fonts/`) и **`GlobalChatAlerts`**.
...
| `frontend/lib/chatClient.ts` | Проверка «у низа» скролла, звук (Web Audio), **Notification**, вибрация; хелперы для чата |
| `frontend/components/GlobalChatAlerts.tsx` | Опрос **`/conversations/unread-count`** вне экранов чата — звук/баннер при росте непрочитанных во вкладке в фоне |
