...
| **API** | Регистрация/логин JWT, вопросы, ответы, лента, сравнение, лайки, личный чат (`/conversations`), групповой чат (`/group-rooms`), **тематические обсуждения** (`/discussions` — посты по осям, комментарии, **ответы на комментарии**), **блокировка и жалоба на пользователя** (`POST/DELETE /users/{id}/block`, `POST /users/{id}/report`), `PATCH /me/profile` |
| **UI** | Тёмная тема, **дизайн-система `mm-*`** (классы в `app/globals.css`), шрифт **Geist**, нижняя навигация с активным разделом; онбординг, тест, summary, **`/terms` и `/privacy`**, регистрация с галочкой согласия, лента, **обсуждения** (список, пост, ввод закреплён на экране на мобильных), диалоги (**личные + группы**), `/chat/[id]` и `/group-chat/[id]` с модерацией где предусмотрено |
...
**Производительность (актуально для Neon):** тяжёлые пути матчинга переведены на **батч-загрузку ответов** (`compute_user_axis_scores_batch` в `core/matching.py`): лента (`GET /feed`), подбор группы (`core/group_matching.py`), блок «общее в комнате» (`core/group_traits.py`). На фронте независимые запросы при открытии чата, группы, ленты и создания поста обсуждения выполняются **параллельно** (`Promise.all`), чтобы не умножать задержку сети. Подробнее — раздел **«Производительность и задержки»** ниже.
...
Миграции: репозиторий содержит цепочку Alembic (в т.ч. групповые таблицы `0007`). Выполняй **`alembic upgrade head`** перед продом и после `git pull`. На **SQLite** при старте API дополнительно подтягиваются недостающие колонки/таблицы (`app/database.py` + `lifespan`), но **вопросы** всё равно нужно залить через `python seed.py`. На **PostgreSQL** без миграций не обойтись.
...
**Обсуждения** (`/discussions`): посты по осям ценностей, комментарии с **reply**; клиент периодически запрашивает новые комментарии (`after_id`), при отсутствии обновлений интервал опроса **увеличивается** (меньше лишних запросов). Правила «кто может комментировать» — `app/core/discussion_access.py`.
...
- **Бэкенд:** для ленты и групп не пересчитываются осевые профили **по отдельности на каждого пользователя в цикле** — один батч ответов на всех нужных `user_id`, затем сравнение в памяти (`compute_user_axis_scores_batch`, `axis_pair_rows_from_scores`, аналогично для группового матчинга и `group_shared_traits_for_user`).
- **Фронт:** на страницах ленты первая загрузка объединяет **`/likes/inbox`**, **`/me/feed-preferences`** и **`/feed`**; на `/chat/[id]` и `/group-chat/[id]` — параллельно **`/auth/me`** и данные чата/комнаты. **`next dev`** компилирует страницы при первом заходе — переходы могут занимать **несколько секунд**; оценка «боевой» скорости: `npm run build` и `npm run start`.
...
1. **Продукт / API:** закрыть пробел **`GET /me/summary`** (если ответ ещё не совпадает с экраном) — см. `TODO.md` фаза B.  
2. **Запуск:** staging/прод, HTTPS, бэкапы БД, базовые **метрики** (см. фаза D в `TODO.md`).  
3. **UX:** второй пак вопросов / баннер «ещё вопросы», полный профиль из `MVP_FLOW`, **a11y** и при желании `next/image` для аватаров.  
4. **После MVP:** OAuth, PWA — см. фаза E в `TODO.md`.
